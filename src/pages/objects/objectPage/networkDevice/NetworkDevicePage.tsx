...
/**
 * Component for displaying information on network device page.
 *
 */
const NetworkDevicePage: FC = () => {
  ...
  const { logs } = useAppSelector(selectNetworkDevice);
  ...
  const {
    ...
    pentesters,
    is_delete: isDelete,
  } = useAppSelector(selectNetworkDeviceById);

  const [isChangeProjectModal, setChangeProjectModal] = useState<boolean>(false);
  const [isDeleteNetworkDeviceModal, setDeleteNetworkDeviceModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (objectId && projectId && name) {
      dispatch(getObjectById({ objectType: OBJECT_TYPES.NetworkDevice, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, name]);

  useEffect(() => {
    if ((role === 'admin' || hasManagerAccess || hasTeamleadAccess) && objectId && projectId) {
      dispatch(getObjectLogs({ objectType: OBJECT_TYPES.NetworkDevice, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, hasManagerAccess, hasTeamleadAccess, role]);

  const removeObjectHandler = () => {
    if (objectId && projectId) {
      dispatch(deleteObject({ projectId, objectId, objectType: OBJECT_TYPES.NetworkDevice }));
    }
  ...
  const onConfirmDeleteModalHandler = () => setDeleteNetworkDeviceModal(prevState => !prevState);
  ...
  const infoList: IItemProps[] = [
    ...
    {
      title: localization.object.filters.networkDeviceText,
      text: networkDeviceName ? networkDeviceName : '-',
      id: 5,
    },
  ];
  ...
  const secondaryInfoList: IItemProps[] = [
    ...
    {
      title: localization.object.filters.assignmentText,
      text: assignmentValue ? assignmentValue : '-',
      id: 5,
    },
  ];
  ...
  return (
    <>
      ...
      <div className={styles['item-content']}>
            ...
            <div className={styles['item-body']}>
              <div className={styles['item-body-info']}>
                <Title className={styles['item-body-info-title']} variant={TitleVariantEnum.H3}>
                  {localization.object.infoNetworkDeviceTitle}
                </Title>
                ...
                <div className={styles['item-panel-top-wrapper']}>
                  <div className={styles['item-panel-top']}>
                    <Title className={styles['item-panel-top-title']} variant={TitleVariantEnum.H3}>
                      <>
                        {localization.object.interactiveNetworkDeviceTitle}
                        {isDelete && (
                          <Archive />
                        )}
                      </>
        ...
        <ConfirmModal
          isModalVisible={isDeleteNetworkDeviceModal}
          setModalVisible={setDeleteNetworkDeviceModal}
        ...
        {(role === 'admin' || hasManagerAccess || hasTeamleadAccess) && (
          <ChangeObject
            ...
            selectTab={OBJECT_TYPES.NetworkDevice}
            selectTitle={OBJECT_TITLES.NETWORK_DEVICE}
          />
        )}
        {(role === 'admin' || hasTeamleadAccess) && (
          <ChangePentester
