...
import {selectExternal, selectExternalById} from '../../../../store/objects/external/externalSelectors';
...
/**
 * Component for displaying information on external page.
 *
 */
const ExternalPage: FC = () => {
  ...
  const { logs } = useAppSelector(selectExternal);
  ...
  const [isChangeExternalModal, setChangeExternalModal] = useState<boolean>(false);
  const [isDeleteExternalModal, setDeleteExternalModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (objectId && projectId && name) {
      dispatch(getObjectById({ objectType: OBJECT_TYPES.External, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, name]);

  useEffect(() => {
    if ((role === 'admin' || hasManagerAccess || hasTeamleadAccess) && objectId && projectId) {
      dispatch(getObjectLogs({ objectType: OBJECT_TYPES.External, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, hasManagerAccess, hasTeamleadAccess, role]);

  const removeObjectHandler = () => {
    if (objectId && projectId) {
      dispatch(deleteObject({ projectId, objectId, objectType: OBJECT_TYPES.External }));
    }

    navigate(`/${ROUTES.PROJECTS}/${projectId}/${ROUTES.OBJECTS}`);
  };

  const onModalHandler = () => setChangeExternalModal(prevState => !prevState);
  const onConfirmDeleteModalHandler = () => setDeleteExternalModal(prevState => !prevState);
  ...
  const secondaryInfoList: IItemProps[] = [
    {
      title: localization.object.filters.ipExternalAddressText,
  ...
  return (
    <>
      ...
      <div className={styles['item-content']}>
            ...
            <div className={styles['item-body']}>
              <div className={styles['item-body-info']}>
                <Title className={styles['item-body-info-title']} variant={TitleVariantEnum.H3}>
                  {localization.object.infoExternalTitle}
                </Title>
                ...
                <div className={styles['item-panel-top-wrapper']}>
                  <div className={styles['item-panel-top']}>
                    <Title className={styles['item-panel-top-title']} variant={TitleVariantEnum.H3}>
                      <>
                        {localization.object.interactiveExternalTitle}
                        {isDelete && (
                          <Archive />
                        )}
                      </>
        ...
        <ConfirmModal
          isModalVisible={isDeleteExternalModal}
          setModalVisible={setDeleteExternalModal}
          text={OBJECT_TITLES.EXTERNAL}
          onConfirmClick={removeObjectHandler}
        />
        {(role === 'admin' || hasManagerAccess || hasTeamleadAccess) && (
          <ChangeObject
            isModalVisible={isChangeExternalModal}
            setModalVisible={setChangeExternalModal}
            selectTab={OBJECT_TYPES.External}
            selectTitle={OBJECT_TITLES.EXTERNAL}
          />
        )}
        {(role === 'admin' || hasTeamleadAccess) && (
          <ChangePentester
