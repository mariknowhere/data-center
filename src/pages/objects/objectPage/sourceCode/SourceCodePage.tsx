...
import {OBJECT_TITLES, OBJECT_TYPES, prepareProgrammingLanguageToRu} from '../../../../constants/objects';
import {selectSourceCode, selectSourceCodeById} from '../../../../store/objects/sourceCodes/sourceCodesSelectors';
...
/**
 * Component for displaying information on source code page.
 *
 */
const SourceCodePage: FC = () => {
  ...
  const { logs } = useAppSelector(selectSourceCode);
  ...
  const [isDeleteSourceCodeModal, setDeleteSourceCodeModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (objectId && projectId && name) {
      dispatch(getObjectById({ objectType: OBJECT_TYPES.SourceCode, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, name]);

  useEffect(() => {
    if ((role === 'admin' || hasManagerAccess || hasTeamleadAccess) && objectId && projectId) {
      dispatch(getObjectLogs({ objectType: OBJECT_TYPES.SourceCode, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, hasManagerAccess, hasTeamleadAccess, role]);

  const removeObjectHandler = () => {
    if (objectId && projectId) {
      dispatch(deleteObject({ projectId, objectId, objectType: OBJECT_TYPES.SourceCode }));
    }
  ...
  const onConfirmDeleteModalHandler = () => setDeleteSourceCodeModal(prevState => !prevState);
  ...
  const secondaryInfoList: IItemProps[] = [
    {
      title: localization.object.filters.numberRowsText,
      text: (numberRows !== null && numberRows !== undefined) ? numberRows.toString() : '-',
      id: 1,
      isFirst: true,
    },
    {
      title: localization.object.filters.programmingLanguageText,
      text: programmingLanguage && programmingLanguage.length !== 0 ?
        preparedMultiSelectData(programmingLanguage, prepareProgrammingLanguageToRu) : '-',
      id: 2,
    },
  ];
  ...
  return (
    <>
      ...
      <div className={styles['item-content']}>
            ...
            <div className={styles['item-body']}>
              <div className={styles['item-body-info']}>
                <Title className={styles['item-body-info-title']} variant={TitleVariantEnum.H3}>
                  {localization.object.infoSourceCodeTitle}
                </Title>
                ...
                <div className={styles['item-panel-top-wrapper']}>
                  <div className={styles['item-panel-top']}>
                    <Title className={styles['item-panel-top-title']} variant={TitleVariantEnum.H3}>
                      <>
                        {localization.object.interactiveSourceCodeTitle}
                        {isDelete && (
                          <Archive />
                        )}
                      </>
        ...
        <ConfirmModal
          isModalVisible={isDeleteSourceCodeModal}
          setModalVisible={setDeleteSourceCodeModal}
        ...
        {(role === 'admin' || hasManagerAccess || hasTeamleadAccess) && (
          <ChangeObject
            ...
            selectTitle={OBJECT_TITLES.SOURCE_CODE}
          />
        )}
        {(role === 'admin' || hasTeamleadAccess) && (
          <ChangePentester
