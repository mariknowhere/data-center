...
import {OBJECT_TYPES, prepareSocialEngineeringTypesToRu} from '../../../../constants/objects';
import {
  selectSocialEngineering,
...
/**
 * Component for displaying information on social engineering page.
 *
 */
const SocialEngineeringPage: FC = () => {
  ...
  const { logs } = useAppSelector(selectSocialEngineering);

  const { name, manager, teamlead } = useAppSelector(selectProjectById);
  ...
  const {
    ...
    pentesters,
    is_delete: isDelete,
  } = useAppSelector(selectSocialEngineeringById);

  const [isChangeProjectModal, setChangeProjectModal] = useState<boolean>(false);
  const [isDeletePerimeterModal, setDeletePerimeterModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (objectId && projectId && name) {
      dispatch(getObjectById({ objectType: OBJECT_TYPES.SocialEngineering, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, name]);

  useEffect(() => {
    if ((role === 'admin' || hasManagerAccess || hasTeamleadAccess) && objectId && projectId) {
      dispatch(getObjectLogs({ objectType: OBJECT_TYPES.SocialEngineering, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, hasManagerAccess, hasTeamleadAccess, role]);

  const removeObjectHandler = () => {
    if (objectId && projectId) {
      dispatch(deleteObject({ projectId, objectId, objectType: OBJECT_TYPES.SocialEngineering }));
    }
  ...
  const onConfirmDeleteModalHandler = () => setDeletePerimeterModal(prevState => !prevState);
  ...
  const infoList: IItemProps[] = [
    ...
    {
      title: localization.object.filters.officeText,
      text: office ? office.name : '-',
      id: 2,
    },
    {
      title: localization.object.filters.engineeringTypeText,
      text: (engineeringType && engineeringType?.length !== 0) ? preparedMultiSelectData(engineeringType, prepareSocialEngineeringTypesToRu) : '-',
      id: 3,
    },
  ];
  ...
  const secondaryInfoList: IItemProps[] = [
    ...
    {
      title: localization.object.filters.successCriterionText,
      text: successCriterion ? successCriterion : '-',
      id: 2,
    },
  ];
  ...
  return (
    <>
      ...
      <div className={styles['item-content']}>
            ...
            <div className={styles['item-body']}>
              <div className={styles['item-body-info']}>
                <Title className={styles['item-body-info-title']} variant={TitleVariantEnum.H3}>
                  {localization.object.infoSocialEngineeringTitle}
                </Title>
                ...
                <div className={styles['item-panel-top-wrapper']}>
                  <div className={styles['item-panel-top']}>
                    <Title className={styles['item-panel-top-title']} variant={TitleVariantEnum.H3}>
                      <>
                        {localization.object.interactiveSocialEngineeringTitle}
                        {isDelete && (
                          <Archive />
                        )}
                      </>
        ...
        <ConfirmModal
          isModalVisible={isDeletePerimeterModal}
          setModalVisible={setDeletePerimeterModal}
          text={'социальную инженерию'}
        ...
        {(role === 'admin' || hasManagerAccess || hasTeamleadAccess) && (
          <ChangeObject
            ...
            selectTab={OBJECT_TYPES.SocialEngineering}
            selectTitle={'социальная инженерия'}
          />
        )}
        {(role === 'admin' || hasTeamleadAccess) && (
          <ChangePentester
