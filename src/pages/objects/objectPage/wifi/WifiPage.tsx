...
import {OBJECT_TITLES, OBJECT_TYPES, prepareAttackerModelToRu} from '../../../../constants/objects';
...
const MobileAppPage: FC = () => {
  ...
  const {
    ...
    pentesters,
    is_delete: isDelete,
  } = useAppSelector(selectWifiById);

  const [isChangeProjectModal, setChangeProjectModal] = useState<boolean>(false);
  const [isDeleteWifiModal, setDeleteWifiModal] = useState<boolean>(false);
  ...
  const onConfirmDeleteModalHandler = () => setDeleteWifiModal(prevState => !prevState);
  ...
  const infoList: IItemProps[] = [
    ...
    {
      title: localization.object.filters.ssidText,
      text: ssidValue ? ssidValue : '-',
      id: 3,
    },
    {
      title: localization.object.filters.bssidText,
      text: bssidValue ? bssidValue : '-',
      id: 4,
    },
  ];
  ...
  return (
    <>
      ...
      <div className={styles['item-content']}>
            ...
            <div className={styles['item-body']}>
              <div className={styles['item-body-info']}>
                <Title className={styles['item-body-info-title']} variant={TitleVariantEnum.H3}>
                  {localization.object.infoWifiTitle}
                </Title>
                ...
                <div className={styles['item-panel-top-wrapper']}>
                  <div className={styles['item-panel-top']}>
                    <Title className={styles['item-panel-top-title']} variant={TitleVariantEnum.H3}>
                      <>
                        {localization.object.interactiveWifiTitle}
                        {isDelete && (
                          <Archive />
                        )}
                      </>
        ...
        <ConfirmModal
          isModalVisible={isDeleteWifiModal}
          setModalVisible={setDeleteWifiModal}
        ...
        {(role === 'admin' || hasManagerAccess || hasTeamleadAccess) && (
          <ChangeObject
            ...
            selectTab={OBJECT_TYPES.WiFi}
            selectTitle={OBJECT_TITLES.WIFI}
          />
        )}
        {(role === 'admin' || hasTeamleadAccess) && (
          <ChangePentester
