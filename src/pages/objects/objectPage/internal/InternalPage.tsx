...
import {selectInternal, selectInternalById} from '../../../../store/objects/internal/internalSelectors';
...
/**
 * Component for displaying information on internal page.
 *
 */
const InternalPage: FC = () => {
  ...
  const { logs } = useAppSelector(selectInternal);
  ...
  const [isChangeInternalModal, setChangeInternalModal] = useState<boolean>(false);
  const [isDeleteInternalModal, setDeleteInternalModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (objectId && projectId && name) {
      dispatch(getObjectById({ objectType: OBJECT_TYPES.Internal, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, name]);

  useEffect(() => {
    if ((role === 'admin' || hasManagerAccess || hasTeamleadAccess) && objectId && projectId) {
      dispatch(getObjectLogs({ objectType: OBJECT_TYPES.Internal, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, hasManagerAccess, hasTeamleadAccess, role]);

  const removeObjectHandler = () => {
    if (objectId && projectId) {
      dispatch(deleteObject({ projectId, objectId, objectType: OBJECT_TYPES.Internal }));
    }

    navigate(`/${ROUTES.PROJECTS}/${projectId}/${ROUTES.OBJECTS}`);
  };

  const onModalHandler = () => setChangeInternalModal(prevState => !prevState);
  const onConfirmDeleteModalHandler = () => setDeleteInternalModal(prevState => !prevState);
  ...
  const secondaryInfoList: IItemProps[] = [
    {
      title: localization.object.filters.ipInternalAddressText,
  ...
  return (
    <>
      ...
      <div className={styles['item-content']}>
            ...
            <div className={styles['item-body']}>
              <div className={styles['item-body-info']}>
                <Title className={styles['item-body-info-title']} variant={TitleVariantEnum.H3}>
                  {localization.object.infoInternalTitle}
                </Title>
                ...
                <div className={styles['item-panel-top-wrapper']}>
                  <div className={styles['item-panel-top']}>
                    <Title className={styles['item-panel-top-title']} variant={TitleVariantEnum.H3}>
                      <>
                        {localization.object.interactiveInternalTitle}
                        {isDelete && (
                          <Archive />
                        )}
                      </>
        ...
        <ConfirmModal
          isModalVisible={isDeleteInternalModal}
          setModalVisible={setDeleteInternalModal}
          text={OBJECT_TITLES.INTERNAL}
          onConfirmClick={removeObjectHandler}
        />
        {(role === 'admin' || hasManagerAccess || hasTeamleadAccess) && (
          <ChangeObject
            isModalVisible={isChangeInternalModal}
            setModalVisible={setChangeInternalModal}
            selectTab={OBJECT_TYPES.Internal}
            selectTitle={OBJECT_TITLES.INTERNAL}
          />
        )}
        {(role === 'admin' || hasTeamleadAccess) && (
          <ChangePentester
