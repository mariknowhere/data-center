...
import {selectWebApp, selectWebAppById} from '../../../../store/objects/webApps/webAppsSelectors';
...
/**
 * Component for displaying information on web app page.
 *
 */
const WebAppPage: FC = () => {
  ...
  const { logs } = useAppSelector(selectWebApp);
  ...
  const {
    ...
    pentesters,
    is_delete: isDelete,
  } = useAppSelector(selectWebAppById);

  const [isChangeProjectModal, setChangeProjectModal] = useState<boolean>(false);
  const [isDeleteWebInterfaceModal, setDeleteWebInterfaceModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (objectId && projectId && name) {
      dispatch(getObjectById({ objectType: OBJECT_TYPES.WebApp, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, name]);

  useEffect(() => {
    if ((role === 'admin' || hasManagerAccess || hasTeamleadAccess) && objectId && projectId) {
      dispatch(getObjectLogs({ objectType: OBJECT_TYPES.WebApp, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, hasManagerAccess, hasTeamleadAccess, role]);

  const removeObjectHandler = () => {
    if (objectId && projectId) {
      dispatch(deleteObject({ projectId, objectId, objectType: OBJECT_TYPES.WebApp }));
    }
  ...
  const onConfirmDeleteModalHandler = () => setDeleteWebInterfaceModal(prevState => !prevState);
  ...
  return (
    <>
      ...
      <div className={styles['item-content']}>
            ...
            <div className={styles['item-body']}>
              <div className={styles['item-body-info']}>
                <Title className={styles['item-body-info-title']} variant={TitleVariantEnum.H3}>
                  {localization.object.infoWebAppTitle}
                </Title>
                ...
                <div className={styles['item-panel-top-wrapper']}>
                  <div className={styles['item-panel-top']}>
                    <Title className={styles['item-panel-top-title']} variant={TitleVariantEnum.H3}>
                      <>
                        {localization.object.interactiveWebAppTitle}
                        {isDelete && (
                          <Archive />
                        )}
                      </>
        ...
        <ConfirmModal
          isModalVisible={isDeleteWebInterfaceModal}
          setModalVisible={setDeleteWebInterfaceModal}
        ...
        {(role === 'admin' || hasManagerAccess || hasTeamleadAccess) && (
          <ChangeObject
            ...
            selectTab={OBJECT_TYPES.WebApp}
            selectTitle={OBJECT_TITLES.WEB_APP}
          />
        )}
        {(role === 'admin' || hasTeamleadAccess) && (
          <ChangePentester
