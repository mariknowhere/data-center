...
import {OBJECT_TITLES, OBJECT_TYPES, prepareMobilePlatformToRu} from '../../../../constants/objects';
import {deleteObject, getObjectById, getObjectLogs} from '../../../../store/objects/objectsAsync';
import {selectMobileApp, selectMobileAppById} from '../../../../store/objects/mobileApps/mobileAppsSelectors';
...
/**
 * Component for displaying information on mobile app page.
 *
 */
const MobileAppPage: FC = () => {
  ...
  const { logs } = useAppSelector(selectMobileApp);
  ...
  const [isDeleteMobileAppModal, setDeleteMobileAppModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (objectId && projectId && name) {
      dispatch(getObjectById({ objectType: OBJECT_TYPES.MobileApp, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, name]);

  useEffect(() => {
    if ((role === 'admin' || hasManagerAccess || hasTeamleadAccess) && objectId && projectId) {
      dispatch(getObjectLogs({ objectType: OBJECT_TYPES.MobileApp, objectId, projectId }));
    }
  }, [dispatch, objectId, projectId, hasManagerAccess, hasTeamleadAccess, role]);

  const removeObjectHandler = () => {
    if (objectId && projectId) {
      dispatch(deleteObject({ projectId, objectId, objectType: OBJECT_TYPES.MobileApp }));
    }
  ...
  const onConfirmDeleteModalHandler = () => setDeleteMobileAppModal(prevState => !prevState);
  ...
  const infoList: IItemProps[] = [
    ...
    },
    {
      ...
      text: platformType ? prepareMobilePlatformToRu[platformType] : '-',
      id: 4,
    },
  ];
  ...
  return (
    <>
      ...
      <div className={styles['item-content']}>
            ...
            <div className={styles['item-body']}>
              <div className={styles['item-body-info']}>
                <Title className={styles['item-body-info-title']} variant={TitleVariantEnum.H3}>
                  {localization.object.infoMobileAppTitle}
                </Title>
                ...
                <div className={styles['item-panel-top-wrapper']}>
                  <div className={styles['item-panel-top']}>
                    <Title className={styles['item-panel-top-title']} variant={TitleVariantEnum.H3}>
                      <>
                        {localization.object.interactiveMobileAppTitle}
                        {isDelete && (
                          <Archive />
                        )}
                      </>
        ...
        <ConfirmModal
          isModalVisible={isDeleteMobileAppModal}
          setModalVisible={setDeleteMobileAppModal}
        ...
        {(role === 'admin' || hasManagerAccess || hasTeamleadAccess) && (
          <ChangeObject
            ...
            selectTab={OBJECT_TYPES.MobileApp}
            selectTitle={OBJECT_TITLES.MOBILE_APP}
          />
        )}
        {(role === 'admin' || hasTeamleadAccess) && (
          <ChangePentester
